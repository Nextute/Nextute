...
const VALIDATION_RULES = {
  ...
  award: { required: true, message: "Award is required" },
};

const LOCAL_STORAGE_KEY = "studentAchievementsForm";
const LOCAL_STORAGE_FILES_KEY = "studentAchievementsFiles";
...
const StudentAchievementsPage = () => {
  ...
  useEffect(() => {
    const fetchSavedData = async () => {
      try {
        ...
        if (
          response.status === 200 &&
          response.data.data?.student_achievements
        ) {
          const parsedStudentAchievements = JSON.parse(
            response.data.data.student_achievements
          );
          const savedAchievements = parsedStudentAchievements.achievements;
          setAchievements(
    ...
    async (idx, e) => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          // ✅ Safely update achievements[idx].documents
          setAchievements((prev) =>
            prev.map((item, i) =>
        ...
        setErrors((prev) => ({
          ...prev,
          [idx]: { ...prev[idx], documents: "Failed to upload documents" },
        }));
      } finally {
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        const response = await axios.patch(
          `${VITE_BACKEND_BASE_URL}/api/institutes/me/student-achievements`,
        ...
        if (response.status === 200) {
          ...
          localStorage.removeItem(LOCAL_STORAGE_KEY);
          localStorage.removeItem(LOCAL_STORAGE_FILES_KEY);
          navigate("/institute/institute-achievements");
        }
      } catch (error) {
